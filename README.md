...
- **Telegram alerts (optional).** Guided setup in Keys: create the bot with @BotFather, message it once, click "Detect chat id" and the runner finds and saves the id itself. Then it tells you what it did while you were away: level reached, sequence complete, halted, started, stopped. Simulated runs are labelled SIMULATED so an alert can never be mistaken for a real fill. Alerts are fire-and-forget: a Telegram outage can never disturb a reconciliation cycle, and no token means no network call at all.

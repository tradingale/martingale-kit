...
- **Pause / resume per sequence.** The site's bot toggle: pausing a sequence makes the automatic checks skip it (Check now and Resync refuse too) while its resting orders stay exactly where they are on the venue. Resume puts the bot back on. Pausing is not stopping: nothing is canceled.
- **Manual sequences.** The site's manual tracking, for orders YOU execute on your own exchange: "Track manually" freezes the same plan at the live entry price and records the level-1 buy as filled, then the runner places nothing at all — you record each fill from the dashboard ("Buy L2 filled", "Sell filled"). Manual records count with real results in the Journal and are deletable, like on the site.
- **Telegram alerts (optional).** Set a bot token and chat id in Keys and the runner tells you what it did while you were away: level reached, sequence complete, halted, started, stopped. Simulated runs are labelled SIMULATED so an alert can never be mistaken for a real fill. Alerts are fire-and-forget: a Telegram outage can never disturb a reconciliation cycle, and no token means no network call at all.
...
- **Four pages.** A nav switches between **Scoreboard** (pick an instrument, preview its sequence, start it), **Dashboard** (active sequences only, with their controls), **Journal** (history and metrics for everything finished), and **Keys & settings**. Key status lives on its own page, not scattered.
